...
def sample_vcon_dict():
    ...
    return {
        ...
        "parties": [
            {"tel": "+1-555-123-4567", "name": "Alice"},
            {"tel": "+1-555-987-6543", "name": "Bob"}
        ],
        "dialog": [
            {
                "type": "recording",
...
class TestGetAudioUrl:
    ...
    def test_get_url_prefers_url_over_filename(self):
        ...
        dialog = {
            "url": "https://example.com/url.wav",
            "filename": "https://example.com/filename.wav"
        }
...
class TestHasTranscription:
    ...
    def test_has_transcription_true(self):
        ...
        mock_vcon = Mock()
        mock_vcon.analysis = [
            {"type": "speechmatics_transcription", "dialog": 0}
        ]
    ...
    def test_has_transcription_different_dialog(self):
        ...
        mock_vcon = Mock()
        mock_vcon.analysis = [
            {"type": "speechmatics_transcription", "dialog": 1}
        ]
    ...
    def test_has_transcription_different_type(self):
        ...
        mock_vcon = Mock()
        mock_vcon.analysis = [
            {"type": "wtf_transcription", "dialog": 0}
        ]
...
class TestRun:
    ...
    def test_run_skip_existing(
        ...
        mock_get_vcon,
        mock_get_redis,
        sample_vcon_with_transcription
    ):
    ...
    @patch('speechmatics_vcon_link.get_redis_connection')
    @patch('speechmatics_vcon_link.get_vcon_from_redis')
    def test_run_skip_non_audio_dialogs(
        self,
        mock_get_vcon,
        mock_get_redis
    ):
...
class TestOptionMerging:
    ...
    @patch('speechmatics_vcon_link.get_redis_connection')
    @patch('speechmatics_vcon_link.get_vcon_from_redis')
    def test_options_merge_with_defaults(
        self,
        mock_get_vcon,
        mock_get_redis
    ):
