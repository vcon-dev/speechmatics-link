...
def run(
    ...
    for index, dialog in enumerate(vcon_obj.dialog):
        ...
        if opts.get("skip_if_exists"):
            if opts.get("save_native_format") and has_transcription(
                vcon_obj, index, "speechmatics_transcription"
            ):
                logger.info(f"Dialog {index} already has native transcription, skipping")
                continue
            
            if opts.get("save_wtf_format") and has_transcription(
                vcon_obj, index, "wtf_transcription"
            ):
