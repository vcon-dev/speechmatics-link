...
def count_low_confidence_words(words: list, threshold: float = 0.5) -> int:
    ...
    return sum(
        1 for w in words
        if w.get("confidence", 1.0) < threshold
    )
        ...
        if result_type == "word":
            ...
            if content in segment_endings and current_segment["text_parts"]:
                ...
                avg_conf = (
                    ...
                    if current_segment["confidences"] else 0.0
                )
...
def build_quality(words: list, segments: list, duration: float) -> dict:
    ...
    word_time = sum(
        ...
        for w in words if not w.get("is_punctuation")
    )
...
def convert_to_wtf(
    ...
    transcript = build_transcript_object(
        speechmatics_response, duration, language, avg_confidence
    )
    
    metadata = build_metadata(
        speechmatics_response, duration, processing_time, created_at
    )
